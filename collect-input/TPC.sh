...
runLevelQA()
{
  ...
  export echunkID=$chunkID
  export eocdbStorage=$ocdbStorage
  cmd="aliroot -b -q -l 'ConfigOCDB.C($runNumber,\"$ocdbStorage\")'  'MakeTrend.C(\"$qaFile\",$runNumber,\"$outfile\")'"
  # echo
  # echo "'MakeTrend.C(\"$qaFile\",\"$runNumber\")'"
  # echo "Parameters passed to MakeTrend:"
  # echo "qaFile = $qaFile"
