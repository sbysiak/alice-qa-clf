int MakeTrend(char *infile, int run, char *outfile) {
  ...
  */
  ...
  TTree *pTreeBeamSpot = (TTree*) cOutputVtxESD->FindObject("fTreeBeamSpot");
  if (!pTreeBeamSpot->GetEntries()) {
      printf("\nERROR - pTreeBeamSpot is not filled in %s - aborting\n", infile);
  }
  ...
  */
  ...
  Int_t returncode = 0;
  AliTPCPerformanceSummary::WriteToFileChunk(pTPC, pTPCgain, pTPCmatch ,pTPCPull,pConstrain, pTreeBeamSpot, outfile, run);
  if (f) { delete f; f=0; }
