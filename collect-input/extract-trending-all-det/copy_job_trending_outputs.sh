...
#  -optional:                                                            #
SOURCE_MAIN=${2:-"/alice/cern.ch/user/s/sbysiak/grid-jobs/OUTPUTS/extractTrending/"}     # std output of grid merging
...
LOG_FILE=${4:-"$HOME/service-task/collect-input/LOGS/copy_job_trending_ouput_$(date "+%m-%d_%H-%M").log"}
#                                                                        #
# # #            # # #           # # #           # # #               # # #
...
FILE_SIZE_THRESHOLD=5  # minimal size over which target file will be removed and copied again (in KB)
TIMEOUT=60               # timeout for alien_cp

cd $TARGET_MAIN  # for eos
for fpath in `alien_find $SOURCE_MAIN/$DATA_DIR "trending_*.root" | grep trending`; do
    # echo
    # echo $fpath
    rel_path=$(echo $fpath | sed 's/extractTrending\// /' | cut -d " " -f 2)
